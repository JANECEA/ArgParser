...
public static class ValidationTests
{
    ...
    public static void ThrowsOnOutOfRange(int age)
    {
        ...
        Assert.Throws<ValidatorFailedException>(() => parser.Parse(ParsingHelper.GetSplitArgs($"-i {age}")));
    }
    ...
    {
        Assert.Throws<WrongValidatorTypeException>(ArgParserFactory.FromType<InvalidValidationArgs>);
    }
    ...
    public static void ThrowsOnUnsatisfiedCustomValidation(string args)
    {
        ...
        Assert.Throws<ValidatorFailedException>(() => parser.Parse(ParsingHelper.GetSplitArgs(args)));
    }   

...
namespace ArgParserTests
{
    public class SimpleTests
    {
        ...
        internal sealed class SimpleArgs_PrimitiveTypes_OptionalOnlyOptions : BaseArgs
        {
            ...
            [ShortNames('i', 'y', 'n'), LongNames("int", "alpha", "bravo"), Help("Integer option"), MetaVarName("INT_VALUE")]
            public int? IntegerOption { get; set; }

            [ShortNames('s', 'g'), LongNames("string", "text", "veryveryverylongoptionname"), Help("String option"), MetaVarName("STR_VALUE")]
        ...
        [
            [new[] { "-i", "stringValue" }],
            [new[] {"-i"}], [new[] {"-y"}], [new[] {"-n"}],
            [new[] {"--int=ahoj"}], [new[] {"--alpha=ahoj"}], [new[] {"--bravo=ahoj"}],
            [new[] {"--int=77a"}], [new[] {"--int=\"\""}],
            [new[] {"--int"}], [new[] {"--alpha"}], [new[] {"--bravo"}]
        ];
        ...
        public void IntTypeParsing_Optional_InvalidInput(string[] args)
        {
            ...
            Assert.Throws<CommandLineParsingException>(()=> { var result = parser.Parse(args); });
        }
        ...
        public void StringTypeParsing_Optional_ValidInput(string[] args, string? Expected_StringOptionResult) 
        {
            var parser = ArgParserFactory.FromType<SimpleArgs_PrimitiveTypes_OptionalOnlyOptions>();
            var result = parser.Parse(args);
        ...
        public void PrimitiveTypes_AllOptional_ValidInput(string[] args, int? Expected_IntegerOptionResult, string? Expected_StringOptionResult, bool Expected_BoolOptionResult)
        {
            var parser = ArgParserFactory.FromType<SimpleArgs_PrimitiveTypes_OptionalOnlyOptions>();
            var result = parser.Parse(args);
        ...
        public void PrimitiveTypes_ShuffledOptions_ValidInput(string[] args, int? Expected_IntegerOptionResult, string? Expected_StringOptionResult, bool Expected_BoolOptionResult)
        {
            var parser = ArgParserFactory.FromType<SimpleArgs_PrimitiveTypes_OptionalOnlyOptions>();
            var result = parser.Parse(args);

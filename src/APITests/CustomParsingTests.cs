...
public static class CustomParsingTests
{
    ...
    {
        Assert.Throws<PropertyNotParsableException>(ArgParserFactory.FromType<PersonUnparsableArgs>);
    }

    private record ParsablePerson(string Name, string Surname, int Age) : Person(Name, Surname, Age), IParsable<ParsablePerson>
    {
        ...
        public static bool TryParse([NotNullWhen(true)] string? s, IFormatProvider? provider, [MaybeNullWhen(false)] out ParsablePerson result)
        {
            var parts = s!.Split(';');
